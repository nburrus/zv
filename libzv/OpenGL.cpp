...
bool gl_checkShader(GLuint handle, const char *desc)
{
    ...
    if ((GLboolean)status == GL_FALSE)
        zv::consoleMessage("ERROR: GLShader: failed to compile %s!\n", desc);
    if (log_length > 1)
    {
        std::vector<char> buf;
        buf.resize((int)(log_length + 1));
...
void GLTexture::upload(const zv::ImageSRGBA& im)
{
    GLRestoreStateAfterScope_Texture _;
...
void GLTexture::download (zv::ImageSRGBA& im)
{
    GLRestoreStateAfterScope_Texture _;
