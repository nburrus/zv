...
bool ImageViewer::initialize (int argc, char** argv, GLFWwindow* parentWindow)
{
    ...
    try
    {
        ...
        bool couldLoad = zv::readPngImage(impl->imagePath, impl->im);
...
void ImageViewer::showImage (const ImageSRGBA& image, const std::string& imagePath)
{
    zv::Rect updatedViewerWindowGeometry;
    impl->imageWindow.showImage (image, imagePath, updatedViewerWindowGeometry);
